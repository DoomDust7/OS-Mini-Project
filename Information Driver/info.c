...
*/
static int pro_usb_probe(struct usb_interface *interface,
...
*/
static void pro_usb_disconnect(struct usb_interface *interface)
{
...
const struct usb_device_id pro_usb_table[] = {
...
MODULE_DEVICE_TABLE(usb, pro_usb_table);
 
//The structure needs to do is register with the linux subsystem
static struct usb_driver pro_usb_driver = {
    .name       = "project usb driver",
    .probe      = pro_usb_probe,
    .disconnect = pro_usb_disconnect,
    .id_table   = pro_usb_table,
};
 
#if ( IS_NEW_METHOD_USED == 0 )
//This will replaces module_init and module_exit.
module_usb_driver(pro_usb_driver);
 
#else
static int __init pro_usb_init(void)
{
    //register the USB device
    return usb_register(&pro_usb_driver);
}
 
static void __exit pro_usb_exit(void)
{
    //deregister the USB device
    usb_deregister(&pro_usb_driver);
}
 
module_init(pro_usb_init);
module_exit(pro_usb_exit);
#endif
 
MODULE_LICENSE("GPL");
MODULE_AUTHOR("project group");
